...
# v24 modifications: stable premium-sale priority after step 144; all v23 routes and quantities retained.
# Inspiration: yamakawanin King RC4 transaction ordering; independently implemented and evaluated.
...
def agent(observation, configuration=None):
    ...
    try:
        ...
        try:
            if _step_of(observation) >= 144:
                market = action.get('market') or []
                def _premium(order):
                    return (len(order) >= 3 and order[0] == 'SELL'
                            and order[1] not in ('WHEAT', 'FERTILIZER')
                            and _int(order[2]) > 0)
                action['market'] = ([o for o in market if _premium(o)]
                                    + [o for o in market if not _premium(o)])
        except Exception:
            pass
        return action
    except Exception:
